...
#so that we can acess
import phonebook_main
import phonebook_gui

def center_window(self,w,h): #pass in the tkinter frame (master) refference and the w and h
...
#catch if user clicks on window upper-right 'X' to ensure they want to close
def ask_quit(self):
...
def addToList(self):
    ...
    # normalize data to keep it consistent in db
    var_fname = var_fname.strip() #will remove any blank spaces before and after user's entry
    var_lname = var_lname.strip() #will ensure that the first char in each word is CAP
    var_fname = var_fname.title()
    var_fname = var_fname.title()
    var_fullname = ("{} {}".format(var_fname, var__lname)) #combine our normalized names into a full name
...
def onDelete(self):
    var_select = self1stList1.get(self.lstList1.curselection()) #Listbox's selected value
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
        # check count to ensure that this is not the last record in db...
        #cannot delete last record or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation", "All information associated with , ({}) \nwill be permenantly deleted from the database. \n\nWould you like to proceed?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_select))
                onDeleted(self) #call the function to clear all txtboxes AND the selected index of listbox
            #### onRefresh(self) #update listbox of changes
                conn.commit()
        else:
...
def onRefresh(self):
    #populate the listbox, ccinciding with the db
...
def onUpdate(self):
    try:
        ...
        var_value = self.lstList1.get(var_select) #list selection's txt value
    except:
        messagebox.showinfo("Missing selection", "No name was selected from the list box. \nThe Update Request has been cancelled.")
        return
    #user will only be allowed to update changes for phone and emails.
    #for name changes, user will need to delete the ENTIRE record and start over.
    ...
    if (len(var_phone > 0)) and (len(var_email > 0)): #ensure that there is data present
        ...
        with conn:
            cur = conn.cursor()
            #count recordds to se iff user's changes are already in
